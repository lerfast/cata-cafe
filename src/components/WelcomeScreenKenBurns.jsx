  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        initial={false}
        animate={prefersReducedMotion ? {} : { scale: [1, 1.06, 1], x: [0, -10, 0], y: [0, -10, 0] }}
      ...
      {/* Contenido */}
      <motion.div
        className="relative container-page flex min-h-[calc(100vh-32px)] flex-col items-center justify-center text-white text-center"
        variants={stagger}
        initial="hidden"
        animate="visible"
      >
        {/* Botón idioma — FIX: respeta notch y es fixed */}
        <motion.button
          className="fixed z-50 rounded-full bg-white/10 px-4 py-2 text-sm font-semibold backdrop-blur
        ...
        <motion.div
          ref={cardRef}
          className="relative w-[92vw] max-w-[680px] rounded-3xl bg-white/10 px-5 py-6 md:px-10 md:py-10 backdrop-blur-lg shadow-2xl ring-1 ring-white/10 will-change-transform"
          variants={fadeIn(0.1)}
        ...
        >
          ...
          {/* Logo (responsive, sin deformar) */}
          <motion.img
            src={logo}
            alt="Cata Café Logo"
            className="mx-auto mb-6 w-[64vw] max-w-[240px] md:max-w-[260px] h-auto rounded-2xl bg-white/10 p-3 shadow-xl ring-1 ring-white/10"
            variants={fadeIn(0.15)}
          />

          {/* Título */}
          <motion.h1
            className="mx-auto max-w-[22ch] bg-gradient-to-r from-amber-300 via-white to-amber-200 bg-clip-text
                       text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent leading-tight"
            variants={fadeIn(0.2)}
          >
          ...
          {/* Subtexto */}
          <motion.p
            className="mx-auto mt-4 max-w-[46ch] text-base text-white/90 md:text-lg"
            variants={fadeIn(0.3)}
          >
          ...
          {/* CTAs */}
          <motion.div className="mt-7 sm:mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row" variants={fadeIn(0.35)}>
            <motion.button
              onClick={onProceed}
              className="inline-flex items-center justify-center rounded-full bg-gradient-to-r from-amber-300 to-amber-400
                         px-6 py-3 font-semibold text-black shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-amber-300/60 active:scale-[0.98] transition"
        ...
        {/* Dirección (más legible en móvil) */}
        <motion.p
          className="mt-6 rounded-xl bg-black/35 px-4 py-2.5 text-sm md:text-base font-medium backdrop-blur ring-1 ring-white/10"
        ...
        <motion.div className="mt-6" variants={fadeIn(0.5)}>
          ...
          <div className="flex items-center justify-center gap-4">
            <a href="https://www.facebook.com/profile.php?id=61559948560646" target="_blank" rel="noopener noreferrer"
               className="grid h-11 w-11 place-items-center rounded-full bg-white/10 ring-1 ring-white/10 hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-white/50 transition" aria-label="Facebook">
              <FontAwesomeIcon className="text-[#1877f2] text-xl" icon={faFacebook} />
            </a>
            <a href="https://www.instagram.com/catacafeneiva/" target="_blank" rel="noopener noreferrer"
               className="grid h-11 w-11 place-items-center rounded-full bg-white/10 ring-1 ring-white/10 hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-white/50 transition" aria-label="Instagram">
              <FontAwesomeIcon className="text-[#e4405f] text-xl" icon={faInstagram} />
            </a>
          </div>
        </motion.div>

        {/* Contador de visitas (respeta safe area bottom/right) */}
